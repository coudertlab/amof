"""
Module containing everythong related to plotting: 
...
def save_hvplot(plot, filename, format = 'svg+png'):
    ...
    """
    if format[0:3] == 'svg':
        output_filename = str(sadi.files.path.append_suffix(filename, 'svg'))
        bp = hv.render(plot)
        bp.output_backend = "svg"
        export_svg(bp, filename=output_filename)
        if format == 'svg+png':
            svg_code = open(output_filename, 'rt').read()
            png_filename = str(sadi.files.path.append_suffix(filename, 'png'))
            svg2png(bytestring=svg_code,write_to=png_filename, scale = 3) # default is 100
    elif format == 'png':
        output_filename = str(sadi.files.path.append_suffix(filename, 'png'))
